...
architecture RTL of SPI_Master_With_Single_CS is
  ...
  signal sig_RX_Count : std_logic_vector(3 downto 0);  -- Index RX byte --TODo:CANsu 3 yap
...
begin
  ...
  SPI_Master_1 : entity work.SPI_Master
    ...
    port map (
      ...
      -- RX (MISO) Signals
      o_RX_DV    => sig_RX_DV,            -- Data Valid pulse
  ...
  begin
    if rising_edge(i_Clk) then
      if r_CS_n = '1' then
        sig_RX_Count <= std_logic_vector(to_unsigned(0, sig_RX_Count'length));

      elsif sig_RX_DV = '1' then
        sig_RX_Count <= std_logic_vector(unsigned(sig_RX_Count) + 1);
      end if;
      
      if (i_TX_DV /= '1' and ((r_SM_CS = IDLE) or (r_SM_CS = TRANSFER and w_Master_Ready = '1' and r_TX_Count > 0))) then
        o_TX_Ready <= '1';
      else
        o_TX_Ready <= '0';
      end if;
